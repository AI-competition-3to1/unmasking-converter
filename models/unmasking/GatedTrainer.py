...
class GatedTrainer():
    def __init__(self,epoch,data_path):
        ...
        self.sample_folder=os.path.join(data_path, 'sample')
        self.trainloader = data.DataLoader(
            self.trainset, 
            batch_size=2,
            collate_fn = self.trainset.collate_fn)
        self.valid_path= os.path.join(data_path, 'valid')
        self.validset=dataset_for_gated(self.valid_path)
        self.validloader = data.DataLoader(
            self.validset, 
            batch_size=2,
        ...
        for epoch in range(self.epoch, self.num_epochs):
            ...
            with torch.no_grad():
                for  i,batch in enumerate(self.trainloader):
                    ...
                    filename = f"{self.epoch}_{str(self.iters)}_{i}"
