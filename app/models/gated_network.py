...
class TransposeGatedConv2d(nn.Module):
    def __init__(
        ...
        self.gated_conv2d = GatedConv2d(
            in_channels, out_channels, kernel_size, stride, padding, dilation, norm=norm
        )
...
class GatedGenerator(nn.Module):
    def __init__(self, in_channels=4, latent_channels=64, out_channels=3):
        ...
        self.coarse = nn.Sequential(
            ...
            GatedConv2d(
                latent_channels * 4, latent_channels * 4, 3, 1, 16, dilation=16
            ),
            GatedConv2d(latent_channels * 4, latent_channels * 4, 3, 1, 1),
            GatedConv2d(latent_channels * 4, latent_channels * 4, 3, 1, 1),
            # decoder
            ...
            GatedConv2d(
                latent_channels, out_channels, 7, 1, 3, activation="tanh", norm=None
            ),
        )
    ...
    def forward(self, inputs, targets):
        ...
        if self.resize:
            inputs = self.transform(
                inputs, mode="bilinear", size=(512, 512), align_corners=False
            )
            targets = self.transform(
                targets, mode="bilinear", size=(512, 512), align_corners=False
            )
        loss = 0.0
        x = inputs
        y = targets
